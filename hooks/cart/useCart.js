...
export default function useCart() {
  ...
  const [
    ...
    {
      loading: isLoading,
      called: accountCartQueryCalled,
      data: cartData,
      fetchMore,
      refetch: refetchAccountCart,
    },
  ...
  const cart = useMemo(() => {
    ...
    cartData,
    cartDataAnonymous,
    shouldSkipAccountCartByAccountIdQuery,
    shouldSkipAnonymousCartByCartIdQuery,
  ])
  ...
  const [addOrCreateCartMutation, { loading: addOrCreateCartLoading }] = useMutation(
    cart && cart._id ? addCartItemsMutation : createCartMutation,
    {
      onCompleted(addOrCreateCartMutationData) {
        if (
          addOrCreateCartMutationData &&
          addOrCreateCartMutationData.createCart &&
          (!viewer || !viewer._id)
        ) {
  ...
  const [removeCartItemsMutationFun, { loading: removeCartItemsLoading }] = useMutation(
    removeCartItemsMutation,
    {
